# Research: Multi-Point Bluetooth Behavior

Status: **Research** — design notes on how multi-point speakers interact with this add-on. No features implemented yet.
...
- **Sending audio to the PA sink** — triggers AVDTP_START, which causes multi-point speakers to switch focus to this source.
...
- **AVRCP PlaybackStatus** — sets the MPRIS player status to "Playing"/"Paused"/"Stopped". Advisory notification to the speaker, not a command.

## What the Speaker Decides (Firmware-Dependent)
...
`MediaTransport1.State` only reflects the state of **this system's own** A2DP transport.
