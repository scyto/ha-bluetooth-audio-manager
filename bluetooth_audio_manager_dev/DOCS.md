...
Note that the dev add-on's hostname ends in `-dev`, so it differs from the
stable add-on's. **Try it out** in the API Reference is not a sandbox — it acts
on your real devices.
