...
class WebServer:
    ...
    def _get_index_html(self) -> str:
        ...
        if self._index_html is None:
            ...
            # Rewrite all static/ asset paths to res/ (bypasses HA service worker)
            raw = raw.replace('"static/', '"res/')
            raw = raw.replace("'static/", "'res/")
            # Add cache-busting version query strings to CSS/JS
            for asset in ("style.css", "app.js"):
                raw = raw.replace(
                    f'res/{asset}"', f'res/{asset}?v={_BUILD_VERSION}"',
                )
